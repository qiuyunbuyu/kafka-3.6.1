...
class ReplicaManager(val config: KafkaConfig,
  ...
  // Purgatory related, help for handling delayed request
  ...
  // epoch of the controller that last changed the leader, Used to isolate requests from expired controllers
  // If the "controllerEpoch" in the "received request" is smaller than the one maintained by itself, the request will be rejected directly
  // May be updated while processing the following requests:
    ...
    def produceStatusResult(appendResult: Map[TopicPartition, LogAppendResult],
      ...
      appendResult.map { case (topicPartition, result) =>
        topicPartition -> ProducePartitionStatus(
          result.info.lastOffset + 1, // count "next record" required offset
  ...
  def fetchMessages(params: FetchParams,
    ...
    // case 1: The conditions for immediate return are met
    if (!remoteFetchInfo.isPresent && (params.maxWaitMs <= 0 || fetchInfos.isEmpty || bytesReadable >= params.minBytes || errorReadingData ||
      ...
      // case2: Cannot be completed immediately, delayed processing
  ...
  /**
   * 成员broker 处理 UpdateMetadataRequest
   */
  def maybeUpdateMetadataCache(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest) : Seq[TopicPartition] =  {
    replicaStateChangeLock synchronized {
      if (updateMetadataRequest.controllerEpoch < controllerEpoch) {
        ...
        // 处理 UpdateMetadataRequest时会更新zkMetadataCache中所维护的 [metadataSnapshot] + 确定deletedPartitions
        val deletedPartitions = zkMetadataCache.updateMetadata(correlationId, updateMetadataRequest)
        controllerEpoch = updateMetadataRequest.controllerEpoch
        // 返回deletedPartitions，后续流程会处理
        deletedPartitions
      }
    }
  }
  ...
  /**
   * handle LeaderAndIsrRequest get from Controller
   * @param correlationId
  ...
  def becomeLeaderOrFollower(correlationId: Int,
    ...
    replicaStateChangeLock synchronized {
      // 1. get broker Id of controller
      val controllerId = leaderAndIsrRequest.controllerId
      // 2. get requestPartitionStates
      ...
      val response = {
        // judge controllerEpoch, ignore expired requests
        if (leaderAndIsrRequest.controllerEpoch < controllerEpoch) {
          ...
          // update controllerEpoch
          controllerEpoch = leaderAndIsrRequest.controllerEpoch

          // create container to store some important variable
          ...
          requestPartitionStates.foreach { partitionState =>
            ...
            // 核心流程1: 从leaderAndIsrRequest封出对应的Partition对象
            // get Partition state: if not found will set "none"
            val partitionOpt = getPartition(topicPartition) match {
              ...
              // case3: None state
              case HostedPartition.None =>
                // 如果收到LeaderAndIsrRequest发现，本地ReplicaManager的allPartitions还没有此topicPartition
                // 则会封装出对应Partition对象， 并添加进allPartitions，并且把本地状态设置为为Online
            ...
            partitionOpt.foreach { partition =>
              // LeaderEpoch from self maintained
              val currentLeaderEpoch = partition.getLeaderEpoch
              // LeaderEpoch from LeaderAndIsrRequest
              val requestLeaderEpoch = partitionState.leaderEpoch

              // TopicId from LeaderAndIsrRequest
              val requestTopicId = topicIdFromRequest(topicPartition.topic)
              // TopicId from local memory
              ...
              if (!hasConsistentTopicId(requestTopicId, logTopicId)) {
                ...
                // Inconsistent, put error response
                responseMap.put(topicPartition, Errors.INCONSISTENT_TOPIC_ID)

              // ======================================= Compare requestLeaderEpoch and currentLeaderEpoch ============
              } else if (requestLeaderEpoch > currentLeaderEpoch) {
                // 核心流程2： 判断对应的Partition对象是成为Leader还是Follower
          ...
          // 核心流程3： makeLeaders
          // Determine which Partition’s Leader Replica is the Replica on the Broker
          ...
          // 核心流程4：makeFollowers
          // Determine which Partition’s Follower Replica is the Replica on the Broker
          ...
          // Remove the corresponding monitoring Metrics
          ...
          // 核心流程5: Schedule执行：["highwatermark-checkpoint", () => checkpointHighWatermarks()]
          ...
          // 核心流程7： 处理LeaderAndIsrRequest时涉及的2个内部topic时身份切换时的回调函数执行
          // Execute callback logic after Leader change
          onLeadershipChange(partitionsBecomeLeader, partitionsBecomeFollower)

          // construct Response for LeaderAndIsrRequest
  ...
  private def makeLeaders(controllerId: Int,
    ...
    partitionStates.keys.foreach { partition =>
      ...
      // use Errors.NONE to init responseMap
    ...
    try {
      ...
      // i am leader now, so i no need to fetch others
    ...
    // Returns the list of partitions that become Leader replicas
   ...
   * 4. Truncate the log and checkpoint offsets for these partitions. [attach leader epoch checkpoint ? ]
   ...
   *
   * 核心流程
   * 核心流程1: 如果metadataCache中知道newLeaderBrokerId，调用Partition的makeFollower
   *        1.1 更新核心变量，follow身份会把remoteReplicasMap给清空掉
   *        1.2 （可能）会调用logManager来创建本地UnifiedLog
   * 核心流程2：先停一把Fetcher
   * 核心流程3：delayed xxx Purgatory处理，清空produce and fetch requests in the purgatory
   * 核心流程4：确定FetchState | 确定Fetch Offset（这里并不是最终的fetchOffset，FetcherThread.doWork(): maybeTruncate()会实际确定fetchOffset）
   * 核心流程5：FetcherThread.doWork()
   */
  private def makeFollowers(controllerId: Int,
    ...
    try {
      // 遍历partitionsToBeFollower
      partitionStates.forKeyValue { (partition, partitionState) =>
        // 确定该TopicPartition的leader replica
      ...
      // 核心流程2：先停一把Fetcher
      // [ Stop fetchers Temporary, Eventually it will start]
      ...
      // 核心流程3：delayed xxx Purgatory处理，清空produce and fetch requests in the purgatory
      // [Clear the produce and fetch requests in the purgatory ]
      ...
      if (isShuttingDown.get()) {
        ...
        val partitionsToMakeFollowerWithLeaderAndOffset = partitionsToMakeFollower.map { partition =>
          ...
          // get local log UnifiedLog
          val log = partition.localLogOrException
          // set fetchOffset：确定Fetch Offset（这里并不是最终的fetchOffset，FetcherThread.doWork(): maybeTruncate()会实际确定fetchOffset）
        ...
        // 核心流程5：FetcherThread.doWork()
        // "i am follower, so i need to fetch..."
        // keep calling FetcherThread.doWork(): maybeTruncate() + maybeFetch()
